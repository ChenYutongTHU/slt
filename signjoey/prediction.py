...
def validate_on_data(
    ...
    with torch.no_grad():
        all_gls_outputs = [] if type(recognition_beam_size)==int else defaultdict(list)
        all_txt_outputs = []
        ...
        for batch in iter(valid_iter):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                batch_recognition_loss, batch_translation_loss, attention = get_loss_for_batch(
                    model=model,
                    batch=batch,
...
def test(
    ...
    if mode=='visualize_attention' and num>0:
    ...
    if "testing" in cfg.keys() and mode=='test':
    ...
    if do_recognition:
        ...
        for rbw in recognition_beam_sizes:
            if dev_recognition_results[rbw]["valid_scores_gathered"]["wer"] < dev_best_wer_score:
                ...
                logger.info("*" * 60)
        if mode=='visualize_attention':
            print(len(dev_recognition_results[rbw]['all_self_attention_scores']))
            print(dev_recognition_results[rbw]['all_self_attention_scores'][0].shape)
            #save
            output_file = '{}.dev.attention.pkl'.format(output_path)
            sgn2attention = {}
            for seq, gls, att in zip(dev_data.sequence, 
                dev_data.gls, 
                dev_recognition_results[rbw]['all_self_attention_scores']):
                #print(seq)
                sgn2attention[seq] = {'gloss':gls, 'attention':att}
            with open(output_file,'wb') as f:
                pickle.dump(sgn2attention,f)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="test",
        type=str,
        help='test or visualize_attention'
    )
    ...
    parser.add_argument(
        ...
        default=100,
        type=int,
        help='effective when mode=visualize_attention'
    )
    ...
    if args.mode == 'visualize_attention':
        output_name += '_attention'
        model_dir = os.path.join(model_dir,'attention')
        make_model_dir(model_dir, overwrite=True)
