...
class UCF101LMDB_2CLIP(object):
    def __init__(self, root='%s/../process_data/data/ucf101' % os.path.dirname(os.path.abspath(__file__)), 
        ...
        self.get_video_id = dict(zip([i.encode('utf-8').decode('utf-8') for i in self.db_order], 
...
class UCF101Flow_LMDB_2CLIP(object):
    def __init__(self, root='%s/../process_data/data/ucf101' % os.path.dirname(os.path.abspath(__file__)), 
        ...
        vname_list = [i.encode('utf-8').decode('utf-8') for i in self.db_order]
        vlen_list_ordered = sorted(list(zip([i.encode('utf-8').decode('utf-8') for i in self.db_keys], self.vlen_list)), key=lambda x: x[0])
